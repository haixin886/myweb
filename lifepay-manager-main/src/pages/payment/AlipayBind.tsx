...
const formSchema = z.object({
  ...
  account: z.string().min(1, "请输入支付宝账号"),
...
const AlipayBind = () => {
  ...
  // Check if user already has Alipay bound
  const { data: existingPayment, isLoading } = useQuery({
    queryKey: ['existing-alipay-payment'],
    queryFn: async () => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      await queryClient.invalidateQueries({ queryKey: ['existing-alipay-payment'] });
      
      toast.success("绑定成功");
      navigate('/payment/settings');
    } catch (error) {
      console.error('Error binding alipay:', error);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white p-4 flex items-center border-b">
        ...
        <h1 className="text-lg font-semibold flex-1 text-center mr-6">绑定支付宝</h1>
      </div>
          ...
          <Card className="p-4">
            ...
            <div className="bg-gray-50 rounded-md p-4 mb-4">
              ...
              <p className="text-sm text-gray-600">
                <span className="font-medium">账号:</span> {existingPayment.account_number}
              </p>
            </div>
